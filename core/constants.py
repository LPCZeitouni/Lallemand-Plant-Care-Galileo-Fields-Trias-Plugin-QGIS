...
DIRECTORY_STRUCTURE = {
    '00_Data': ['00_Raw_Files', '01_Reproject', '02_Sampling'],
    '01_Kriging': [
        '01_T1_T2_Total',
        '02_T1_Total',
        '03_T2_Total',
    ...
    '03_Error_Compensation': [
        'T1_Error_Compensation',
        'T2_Error_Compensation'
    ],
    '04_Gain_Surface': [],
    '05_Results': [
        '01_Histograms',
...
INSERT_CROP_SQL = (
    "INSERT INTO crop_trial "
    "(crop_name, sowing_date, harvest_date, variety, inter_ro_cm, create_date) "
    "VALUES (?, ?, ?, ?, ?, ?);"
)

UPDATE_CROP_SQL = (
    "UPDATE crop_trial SET crop_name = ?, sowing_date = ?, harvest_date = ?, "
    "variety = ?, inter_ro_cm = ?, update_date = ? WHERE id = ?;"
)
...
INSERT_FARMER_SQL = (
    "INSERT INTO farmer "
    "(first_name, last_name, address, zipcode, town, country, create_date) "
    "VALUES (?, ?, ?, ?, ?, ?, ?);"
)

UPDATE_FARMER_SQL = (
    "UPDATE farmer SET first_name = ?, last_name = ?, address = ?, "
    "zipcode = ?, town = ?, country = ?, update_date = ? WHERE id = ?;"
)
...
INSERT_TRIAL_SQL = (
    "INSERT INTO geostatistic_trial "
    "(field_name, field_area, field_irrigation, field_soil, lpc_team_id, "
    "farmer_id, crop_trial_id, id_contour, create_date) "
    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);"
)

UPDATE_TRIAL_SQL = (
    "UPDATE geostatistic_trial SET field_name = ?, field_area = ?, "
    "field_irrigation = ?, field_soil = ?, lpc_team_id = ?, farmer_id = ?, "
    "crop_trial_id = ?, id_contour = ?, update_date = ? WHERE id = ?;"
)

DELETE_TRIAL_SQL = "DELETE FROM geostatistic_trial WHERE id = '{}';"

FETCH_ALL_TEAM = (
    "SELECT id, first_name, last_name, create_date, update_date FROM lpc_team;"
)

FETCH_ONE_TEAM = (
    "SELECT id, first_name, last_name, create_date, update_date "
    "FROM lpc_team WHERE id = '{}';"
)

INSERT_TEAM_SQL = (
    "INSERT INTO lpc_team (first_name, last_name, create_date) "
    "VALUES (?, ?, ?);"
)

UPDATE_TEAM_SQL = (
    "UPDATE lpc_team SET first_name = ?, last_name = ?, update_date = ? "
    "WHERE id = ?;"
)
...
COMPOSER_LAYOUTS = {
    ...
    # Final comparable surfaces.
    # Existing QPT templates are reused intentionally.
    'T1_Final_Surface': '07_Model_T1.qpt',
    'T2_Final_Surface': '08_Model_T2.qpt',
