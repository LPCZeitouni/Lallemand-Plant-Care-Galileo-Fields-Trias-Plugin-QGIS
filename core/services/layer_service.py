...
class LayerService:
    ...
    def yieldGainFrequencyHistogram(self, layer, path, thresholds=None, colors=None):
        """
        Génère l'histogramme de gain de rendement avec couleurs et seuils.
        """
        values = list()
        ...
        if len(values) > 0:
            # On envoie les valeurs au PlotterService avec les paramètres de style
            self.plotterService.yieldFrequencyHistogram(
                values, 
                exportPng=True, 
                path=path, 
    ...
    def applyGainSymbology(self, layer, thresholds, colors):
        """
        Applique une symbologie par classes fixes (Discrete) au raster de gain.
        """
        shader = QgsRasterShader()
        color_ramp_shader = QgsColorRampShader()
        
        # Mode Discrete pour avoir des plages de couleurs bien nettes
        color_ramp_shader.setColorRampType(QgsColorRampShader.Discrete)
        
        # Création des items de la légende
        # thresholds[0]=0, thresholds[1]=5, thresholds[2]=15
        items = [
            QgsColorRampShader.ColorRampItem(thresholds[0], QColor(colors[0]), "< 0%"),
            QgsColorRampShader.ColorRampItem(thresholds[1], QColor(colors[1]), "0 - 5%"),
            QgsColorRampShader.ColorRampItem(thresholds[2], QColor(colors[2]), "5 - 15%"),
            QgsColorRampShader.ColorRampItem(999999, QColor(colors[3]), "> 15%")
        ]
